...
object AudioCache {
    ...
    // The Polestar 3 grants a 64 MB cache quota (measured) — about eight songs of AAC-256.
    // The quota is advisory (exceeding it only moves us up the OS's purge order, it doesn't
    // break anything), so refuse to go below a floor that makes the streaming cache useful.
    // Offline listening is the Downloads feature's job, not this cache's.
    private const val MIN_CACHE_BYTES = 256L * 1024 * 1024
    ...
    // Between 256 MB and 5 GiB, guided by the system's fair-share cache quota: staying under
    // the quota means the OS clears this app's cache last under storage pressure. The quota is
    ...
    private fun cacheSizeFor(context: Context): Long {
        return try {
            ...
            quota.coerceIn(MIN_CACHE_BYTES, MAX_CACHE_BYTES).also {
